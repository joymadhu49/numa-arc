    ...
    return (
      ...
      >
        ...
        <span className="hidden md:inline">
          {auth.connectPending ? 'Connecting…' : 'Connect Wallet'}
        </span>
      </Button>
    )
  }
  ...
  return (
    <div ref={ref} className="relative w-full">
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        className={cn(
          'flex w-full items-center justify-between gap-2 rounded-md border border-neutral-800 bg-neutral-900 px-3 py-2 text-sm text-neutral-100 transition-colors hover:bg-neutral-800',
        )}
      >
        <span className="flex items-center gap-2 truncate">
          <span className="h-2 w-2 shrink-0 rounded-full bg-emerald-500" aria-hidden />
          <span className="hidden truncate md:inline">{truncate(auth.address)}</span>
        </span>
        <ChevronDown className="hidden h-3.5 w-3.5 text-neutral-400 md:inline" />
      </button>

      {open ? (
        <div className="absolute right-0 top-full z-50 mt-2 w-44 overflow-hidden rounded-md border border-neutral-800 bg-neutral-950 shadow-lg">
          <button
            type="button"
