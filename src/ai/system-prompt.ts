...
const NETWORK_POSTURE = MAINNET_ENABLED
  ? `- Mainnet networks are ENABLED (Arc mainnet chainId 5042, Ethereum, Base, and other CCTP mainnets) alongside the testnets. Real funds may be involved on mainnet chains — be extra explicit about network, amounts, and risk. Arc MAINNET swap/send/bridge are not yet supported by Circle App Kit and will fail with a clear error; testnet remains the default.`
...
- 10+ language support.
- No emojis.
- Never use the em dash character (—) in replies. Use commas, colons, or periods instead.
...
- NATIVE CCTP bridges (any bridge where one end has no App Kit support; today that means Arc MAINNET) mint in a second step: after the burn broadcasts, tell the user the claim comes after Circle attests (~15 min for standard). When get_bridge_status reads ready_to_mint, call claim_bridge with fromChain, toChain, and the burn txHash. App Kit bridges mint automatically; never call claim_bridge for them.
